...
Pro-category는 어떤 category $\mathscr C$의 objects를 co-directed projective limit 형태로 완성해서 다루는 category이다. 이 page에서는 일반 pro-category 이론 전체가 아니라, [[topics/affine-objects-in-monoidal-categories|Affine Objects in Monoidal Categories]]를 읽는 데 필요한 최소한의 $\operatorname{Pro}(\mathscr C)$ language만 다룬다.
...
Affine object를 위해서는 다음 coherent pro-object subcategory도 필요하다. $A=\bigoplus_{k\in\mathbb Z}A_k$를 nonnegatively graded commutative $\mathbf k$-algebra라고 하자. $\operatorname{Mod}^{\mathrm{gr}}(A,\operatorname{Pro}(\mathscr C))$는 $\operatorname{Pro}(\mathscr C)$ 안의 graded $A$-modules로 이루어진 category이다.
...
- $\mathscr C$는 Yoneda functor를 통해 $\operatorname{Pro}(\mathscr C)$ 안에 fully faithfully 들어간다.
- 이 finite-length graded setting에서는 $\mathscr C$가 $\operatorname{Pro}(\mathscr C)$ 안의 full subcategory로 쓰이며, subquotients를 취해도 $\mathscr C$ 안에 남는다.
...
- $A=\mathbf k[z]$인 경우 이 language가 [[topics/affine-objects-in-monoidal-categories|affine object]]의 $z$-adic completeness condition을 표현한다.

## 다른 topic들과의 관계

- [[topics/affine-objects-in-monoidal-categories|Affine Objects in Monoidal Categories]]는 $\operatorname{Pro}^{\mathrm{coh}}(\mathbf k[z],\mathscr C)$ 안에서 affine object $(\widehat M,z)$를 정의한다.
- [[topics/graded-monoidal-categories|Graded Monoidal Categories]]는 pro-category language 위에 tensor product와 grading shift $q$를 함께 놓는 ambient setting을 제공한다.
- [[topics/category-localization|Category Localization]]도 category를 더 큰 category로 옮겨서 object와 morphism을 다룬다는 점에서는 관련이 있지만, pro-category completion과 localization은 같은 construction이 아니다.

## 더 읽을 topic

- 먼저 읽을 것: 이 page 자체가 affine-object layer를 위한 categorical prerequisite이다.
- 상위 개념: 별도의 상위 topic 없이 categorical prerequisite의 출발점으로 읽는다.
- 다음에 읽을 것: [[topics/affine-objects-in-monoidal-categories|Affine Objects in Monoidal Categories]]에서는 $\operatorname{Pro}^{\mathrm{coh}}(\mathbf k[z],\mathscr C)$를 읽고, [[topics/graded-monoidal-categories|Graded Monoidal Categories]]에서는 tensor와 grading structure를 읽는다.
