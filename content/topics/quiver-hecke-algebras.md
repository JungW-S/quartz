...
Quiver-Hecke algebra, 또는 KLR algebra는 quiver와 그 quiver에서 얻는 Cartan datum에 붙는 graded algebra family이다. 하나의 positive root $\alpha\in Q_+$마다 algebra $H_\alpha$가 있고, 이 algebra들의 [[topics/quiver-hecke-module-categories|module categories]]가 [[topics/quantum-groups|Quantum Groups]]의 한쪽 절반과 [[topics/monoidal-categorification|Monoidal Categorification]]을 연결한다.
...
Monoidal categorification에서는 algebra multiplication을 module category의 convolution product로 해석한다. Quiver-Hecke algebra의 finite-dimensional graded modules는 [[topics/quiver-hecke-module-categories|module category]]를 만들고, 그 안의 [[topics/quiver-hecke-subcategories|subcategories]]가 [[topics/determinantial-modules|Determinantial Modules]]와 [[topics/quiver-hecke-category-localization|localized categories]]가 놓이는 category-level 배경을 제공한다.
...
$$
Generator $\theta_i$는 projective module class $[H_{\alpha_i}]$로 간다. 이 명제는 quiver-Hecke algebra가 [[topics/quantum-groups|Quantum Groups]]의 한쪽 절반을 categorify한다는 정확한 형태이다.

### 해석

Finite-dimensional graded $H_\alpha$-modules의 categories를 모으면 induction과 restriction을 통해 multiplication과 comultiplication을 갖는다. 이 구조가 [[topics/quiver-hecke-module-categories|Quiver-Hecke Module Categories]], real simple modules, [[topics/determinantial-modules|Determinantial Modules]], [[topics/quiver-hecke-category-localization|Quiver-Hecke Category Localization]]을 다룰 때 쓰이는 category-level 배경이다.
...
- [[topics/quantum-groups|Quantum Groups]]는 quiver-Hecke projective modules의 Grothendieck group에서 회복되는 quantum-group-level structure를 제공한다.
- [[topics/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]는 $H_\alpha$의 modules를 모은 category-level 중간 층이다.
- [[topics/quiver-hecke-subcategories|Quiver-Hecke Subcategories]]는 $\mathcal C_w$와 $\mathcal C_{w,v}$처럼 localization과 determinantial modules가 실제로 놓이는 subcategories를 제공한다.
- [[topics/determinantial-modules|Determinantial Modules]]는 quiver-Hecke subcategories 안의 distinguished module objects이다.
- [[topics/quiver-hecke-category-localization|Quiver-Hecke Category Localization]]은 $\mathcal C_w$에서 출발해 determinantial objects를 invertible하게 만드는 construction이다.
...
- 먼저 읽을 것: [[topics/root-systems-and-weight-lattices|Root Systems and Weight Lattices]]에서 root와 Cartan notation을 먼저 읽는다.
- 상위 개념: 별도의 상위 topic 없이 Quiver-Hecke algebra 쪽의 출발점으로 읽는다.
- 다음에 읽을 것: [[topics/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]에서는 $R\text{-gmod}$를 읽고, [[topics/quiver-hecke-subcategories|Quiver-Hecke Subcategories]]에서는 $\mathcal C_w$와 $\mathcal C_{w,v}$를 읽는다. [[topics/monoidal-categorification|Monoidal Categorification]]에서는 Grothendieck-ring interpretation을 읽는다.
