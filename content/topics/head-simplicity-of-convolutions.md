...
M\circ N
$$
의 head와 socle이 언제 simple object로 통제되는지를 말하는 theorem-level topic이다. 핵심 상황은 한쪽 factor가 real simple처럼 행동하고 다른 factor가 simple일 때이다. 이때 renormalized R-matrix의 image가 단순히 어떤 subquotient가 아니라, 한 방향 product의 head와 반대 방향 product의 socle을 정확히 식별한다.

이 정리는 [[topics/normal-sequences|Normal Sequences]] 뒤에 자연스럽게 나온다. Normal sequence는 여러 simple objects의 ordered convolution을 다루고, head simplicity theorem은 그런 ordered product에서 왜 simple head가 안정적인 대상이 되는지를 설명한다.

Localized crystal construction에서는 head convolution notation
$$
M\nabla N:=\operatorname{hd}(M\circ N)
$$
이 반복해서 등장한다. Head simplicity theorem은 이 notation이 임의의 quotient가 아니라 R-matrix와 연결된 구조적 simple object를 가리킨다는 배경을 제공한다.
...
$$
로 쓴다. 이 notation의 construction과 degree는 [[topics/r-matrix-renormalization|R-Matrix Renormalization]]에서 먼저 읽는다.
...
$$
꼴로 둔다. Symmetric quiver-Hecke algebra setting에서는 KKKO15 Corollary 3.3에 의해 이 조건이 $M$의 real simplicity와 동치이다.
...
핵심 그림은 두 방향 R-matrix의 image가 서로 다른 product의 head와 socle을 교차해서 잡는다는 것이다.
...
이 morphism의 image는 왼쪽 source $N\circ M$에서는 head이고, 오른쪽 target $M\circ N$에서는 socle이다.
...
N\circ M
$$
의 image는 $M\circ N$의 head이고, $N\circ M$의 socle이다.

따라서 head simplicity theorem은 category-level statement이다. Grothendieck ring에서는 product $[M][N]$의 distinguished simple constituent를 추적하는 것처럼 보이지만, 실제 내용은 convolution product 안의 morphism image가 head와 socle을 동시에 식별한다는 것이다.
...
$d_i(M)>0$이면 head convolution after multiplying by $L(i)$는 $d_i$를 하나 줄인다.
$$
d_i(L(i)\nabla M)=d_i(M\nabla L(i))=d_i(M)-1.
$$
...
- [[topics/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]는 $R(\beta)\text{-mod}$, convolution product $\circ$, simple modules의 ambient category를 제공한다.
- [[topics/r-matrix-renormalization|R-Matrix Renormalization]]은 $r_{M,N}$와 R-matrix degree를 설명한다.
- [[topics/normal-sequences|Normal Sequences]]는 head/socle theorem을 여러 factors의 ordered product로 확장해서 쓰는 조건이다.
- [[topics/localized-root-operators|Localized Root Operators]]는 $\nabla$를 사용해 localized category의 crystal operators를 정의한다.
- [[topics/root-objects-in-localized-categories|Root Objects in Localized Categories]]는 localized root operator의 input object가 갖추어야 할 R-matrix degree 조건을 다룬다.

## 더 읽을 topic

- 먼저 읽을 것: [[topics/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]에서 convolution product를 읽고, [[topics/r-matrix-renormalization|R-Matrix Renormalization]]에서 renormalized R-matrix를 읽는다.
- 상위 개념: [[topics/normal-sequences|Normal Sequences]]는 이 theorem을 ordered convolution product로 확장해서 쓰는 주변 개념이다.
- 다음에 읽을 것: [[topics/localized-root-operators|Localized Root Operators]]에서 $M\nabla N$ notation이 crystal operator formulas에 들어가는 방식을 읽고, [[topics/shuffle-lemmas-for-quiver-hecke-modules|Shuffle Lemmas for Quiver-Hecke Modules]]에서 head/socle와 restriction functor가 함께 쓰이는 다음 단계 lemmas를 읽는다.
