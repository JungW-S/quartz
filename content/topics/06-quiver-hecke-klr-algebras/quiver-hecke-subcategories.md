...
[[topics/06-quiver-hecke-klr-algebras/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]에서 [[topics/08-localization-of-categories/quiver-hecke-category-localization|Quiver-Hecke Category Localization]]으로 넘어가려면 먼저 $R\text{-gmod}$ 안의 $\mathcal C_w$나 $\mathcal C_{w,v}$를 구분해야 한다. 그래야 그 다음에 어떤 category를 localization하는지 말할 수 있다.
...
여기서 쓰는 category notation은 다음과 같다.
...
$\mathcal C_w$, $\mathcal C_{*,v}$, $\mathcal C_{w,v}$는 subquotients, extensions, convolution products, grading shifts에 대해 닫혀 있다. 이 닫힘 성질 때문에 이 subcategories는 monoidal categorification에서 사용할 수 있는 category-level 환경이 된다.

### Grothendieck-ring comparison

Grothendieck-ring identification 아래에서 $K_0(\mathcal C_{w,v})$는 $A_{w,v}$와 비교된다. 여기서 $K_0(\mathcal C_{w,v})$는 category에서 내려온 ring이고, $A_{w,v}$는 coordinate-ring-level algebra이다.
...
**Quiver-Hecke Module Categories.** [[topics/06-quiver-hecke-klr-algebras/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]는 ambient category $R\text{-gmod}$를 제공한다. $\mathcal C_w$, $\mathcal C_{*,v}$, $\mathcal C_{w,v}$는 이 category 안에서 정의되는 full subcategories이다.

**Determinantial Modules.** [[topics/06-quiver-hecke-klr-algebras/determinantial-modules|Determinantial Modules]]는 $\mathcal C_{w,v}$ 같은 subcategory 안에서 구별되는 objects를 다룬다.

**Quiver-Hecke Category Localization.** [[topics/08-localization-of-categories/quiver-hecke-category-localization|Quiver-Hecke Category Localization]]은 $\widetilde{\mathcal C}_w$로 가기 전에 $\mathcal C_w$를 category-level input으로 사용한다.

**Quantum Coordinate Rings.** [[topics/01-quantum-groups/quantum-coordinate-rings|Quantum Coordinate Rings]]는 $A_{w,v}$와 $A_q(\mathfrak n(w))$ 같은 notation이 놓이는 coordinate-ring side를 제공한다.

**Monoidal Categorification.** [[topics/05-monoidal-categorification/monoidal-categorification|Monoidal Categorification]]은 monoidal category의 Grothendieck ring을 cluster algebra나 coordinate algebra와 비교하는 이유를 설명한다.

## 더 읽을 topic

- 먼저 읽을 것: $R\text{-gmod}$를 보려면 [[topics/06-quiver-hecke-klr-algebras/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]를 읽고, coordinate-ring side를 보려면 [[topics/01-quantum-groups/quantum-coordinate-rings|Quantum Coordinate Rings]]를 읽는다.
- 상위 개념: [[topics/06-quiver-hecke-klr-algebras/quiver-hecke-module-categories|Quiver-Hecke Module Categories]]가 더 넓은 category-level 배경이다.
- 다음에 읽을 것: $\mathcal C_{w,v}$ 안의 objects는 [[topics/06-quiver-hecke-klr-algebras/determinantial-modules|Determinantial Modules]]에서 이어지고, $\mathcal C_w$의 localization은 [[topics/08-localization-of-categories/quiver-hecke-category-localization|Quiver-Hecke Category Localization]]에서 이어진다.
