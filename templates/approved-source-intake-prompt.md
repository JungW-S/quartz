...
9. Update `data/source_candidates.yml`, `data/topic_maturity.yml`, and `data/research_queue.yml`; record intentionally unfilled overview, setup/notation, definition, example, viewpoint, property, mechanism, connection, navigation, or source-note components rather than inventing them.
   If an example is added, label it as `검증: 논문 예시`, `검증: Sage 계산`, or `검증: 논문 그림` directly under the example and record detailed provenance in `Source notes`.
...
- Do not force examples. Visible examples require an approved source location or checked Sage code stored in this repository; schematic examples must be labeled `구조 예시`, explain only a general mechanism, and still be paper- or Sage-verified.
- Do not use LLM-generated mathematical examples or LLM-generated mathematical figures as topic-page examples unless they have been independently verified by an approved source or checked Sage code.
- Visual examples must be captured/converted from an approved source figure/table/page or generated by checked Sage code after mathematical verification.
...
- example verification notes, including exact source location, Sage command/code path, generated image path, or paper figure/page path when a visible example is used
